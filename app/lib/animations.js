...
var animations = function animations(inJobs) {
  var inJobs = inJobs;
  var robot = '#robot',
      $robot = $('#robot'),
      waypoint = '#waypoint',
  ...
  var animateJobGroup = function animateJobGroup(jobGroup) {
    reset();
  ...
  function takeItem(itemNr, callback) {
    if (lastItem === itemNr) {
      ...
      lastItem = itemNr;
      move('#item-' + itemNr).set('opacity', '0').duration('0.3s').end(callback);
    }
  }
  ...
  var moveToNextWaypoint = function moveToNextWaypoint(waypointNr, callback) {
    ...
    function moveRightBy(pixels) {
      move(robot).add('margin-left', pixels).end(triggerNextAnimation);
    }

    function moveDownBy(pixels) {
      move(robot).add('margin-top', pixels).end(triggerNextAnimation);
    }

    function moveUpBy(pixels) {
      move(robot).sub('margin-top', pixels).end(triggerNextAnimation);
    }
  };
  ...
  function moveToEnd(callback) {
    var $lastWaypointInFirstLane = $('#waypoint-' + grid.getSlotsInLane() / 2),
        ...
        moveLeft = move(robot).sub('margin-left', offsetRight).then(callback);

    move(robot).add('margin-top', offsetBottom).then(moveLeft).end();
  }
